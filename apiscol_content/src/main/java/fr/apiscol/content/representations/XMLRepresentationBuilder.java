...
import fr.apiscol.content.crawler.LinkRefreshingHandler.State;
import fr.apiscol.content.databaseAccess.DBAccessBuilder;
import fr.apiscol.content.databaseAccess.DBAccessBuilder.DBTypes;
...
import fr.apiscol.content.recovery.ContentRecoveryHandler;
import fr.apiscol.content.recovery.ContentRecoveryHandler.MessageTypes;
...
public class XMLRepresentationBuilder extends AbstractRepresentationBuilder<Document> {
	...
	@Override
	public Document getResourceRepresentation(URI baseUri, String apiscolInstanceName, String resourceId,
			String editUri) throws DBAccessException, InexistentResourceInDatabaseException {
		Document XMLRepresentation = XMLUtils.createXMLDocument();

		addXMLSubTreeForResource(XMLRepresentation, XMLRepresentation, baseUri, apiscolInstanceName, resourceId,
				editUri);
		XMLUtils.addNameSpaces(XMLRepresentation, UsedNamespaces.ATOM);
		return XMLRepresentation;
	}

	private void addXMLSubTreeForResult(Document XMLDocument, Node insertionElement, String resourceId, String fileName,
			String score, String type, List<String> snippets, String apiscolInstanceName) {
		Element rootElement = XMLDocument.createElement("apiscol:hit");
		insertionElement.appendChild(rootElement);
		rootElement.setAttribute("resourceId", getResourceUrn(resourceId, apiscolInstanceName));
	...
	private void addXMLSubTreeForSpellcheck(Document XMLDocument, Element insertionElement,
			Map<String, List<String>> suggestionsforTerms, List<String> suggestionsforQuery) {
		Iterator<String> it = suggestionsforTerms.keySet().iterator();
		String term;
		while (it.hasNext()) {
			term = it.next();
			Element queryTermElement = XMLDocument.createElement("apiscol:query_term");
	...
	private long addXMLSubTreeForResource(Document XMLDocument, Node insertionElement, URI baseUri,
		...
		long updateTime;
		String etagForResource = getEtagForResource(resourceId).replace("data-version-", "");
		try {
			updateTime = Long.parseLong(etagForResource);
		} catch (NumberFormatException e1) {
			logger.warn(String.format("Etag for ressourse %s is not a valid long : %s", resourceId, etagForResource));
			updateTime = 0L;
		}
		...
		idElement.setTextContent(getResourceUrn(resourceId, apiscolInstanceName));
		...
		if (StringUtils.isNotEmpty(editUri)) {
			...
			editElement.setAttribute("href", getResourceEditUri(editUri, resourceId));
			...
			editMediaElement.setAttribute("href", getResourceEditMediaUri(editUri, resourceId, type));
		...
		selfHTMLLinkElement.setAttribute("href", getResourceHTMLUri(baseUri, resourceId));
		...
		selfAtomXMLLinkElement.setAttribute("href", getResourceAtomXMLUri(baseUri, resourceId));
		...
		selfThumbLinkElement.setAttribute("href", getResourceThumbsUri(baseUri, resourceId));
		...
		selfPreviewLinkElement.setAttribute("href", getResourcePreviewUri(baseUri, resourceId));
		...
		if (type.equals(ContentType.url)) {
			...
			Element htmlLinkElement = XMLDocument.createElementNS(UsedNamespaces.XHTML.getUri(), "a");
			...
			try {
				...
				while (it.hasNext()) {
					addXMLSubTreeForFile(XMLDocument, filesElement, baseUri, apiscolInstanceName, resourceId,
							it.next());
				}
			} catch (ResourceDirectoryNotFoundException e) {
				logger.warn(String.format("Directory not found for resource %s with message %s", resourceId,
						e.getMessage()));
				System.out.println(String.format("Directory not found for resource %s with message %s", resourceId,
						e.getMessage()));
			...
			if (ResourceDirectoryInterface.existResourceArchive(resourceId)) {
				Element downloadLinkElement = XMLDocument.createElement("apiscol:archive");
				downloadLinkElement.setAttribute("type", "application/zip");
				downloadLinkElement.setAttribute("src", getResourceArchiveDownloadUri(baseUri, resourceId));
	...
	private void addXMLSubTreeForFile(Document document, Node insertionNode, URI baseUri, String apiscolInstanceName,
		...
		downloadLinkElement.setAttribute("href", getFileDownloadUrl(baseUri, resourceId, fileName));
	...
	@Override
	public Document getFileSuccessfulDestructionReport(URI baseUri, String apiscolInstanceName, String resourceId,
			String fileName) {
		...
		Element linkElement = report.createElementNS(UsedNamespaces.ATOM.getUri(), "link");
		linkElement.setAttribute("href", getFileDownloadUrl(baseUri, resourceId, fileName));
	...
	@Override
	public Document getResourceSuccessfulDestructionReport(URI baseUri, String apiscolInstanceName, String resourceId,
			String warnings) {
		Document report = XMLUtils.createXMLDocument();
		Element rootElement = report.createElement("status");
		Element stateElement = report.createElement("state");
		...
		stateElement.setTextContent("done");
		Element linkElement = report.createElementNS(UsedNamespaces.ATOM.getUri(), "link");
		linkElement.setAttribute("href", getResourceAtomXMLUri(baseUri, resourceId));
		Element messageElement = report.createElement("message");
	...
	@Override
	public Document getResourceUnsuccessfulDestructionReport(URI baseUri, String apiscolInstanceName, String resourceId,
			String warnings) {
		Document report = XMLUtils.createXMLDocument();
		Element rootElement = report.createElement("status");
		Element stateElement = report.createElement("state");
		...
		Element linkElement = report.createElementNS(UsedNamespaces.ATOM.getUri(), "link");
		linkElement.setAttribute("href", getResourceArchiveDownloadUri(baseUri, resourceId));
		Element messageElement = report.createElement("message");
	...
	@Override
	public Document getInexistentFileDestructionAttemptReport(URI baseUri, String resourceId, String fileName) {
		return null;
	}

	@Override
	public Document getCompleteResourceListRepresentation(URI baseUri, String apiscolInstanceName, int start, int rows,
		...
		Element totalElement = response.createElement(UsedNamespaces.OPENSEARCH.getShortHand() + ":totalResults");
		totalElement.setTextContent(Integer.toString(resourcesList.size()));
		rootElement.appendChild(totalElement);
		Element startElement = response.createElement(UsedNamespaces.OPENSEARCH.getShortHand() + ":startIndex");
		startElement.setTextContent(Integer.toString(start));
		rootElement.appendChild(startElement);
		Element rowsElement = response.createElement(UsedNamespaces.OPENSEARCH.getShortHand() + ":itemsPerPage");
		...
		while (it.hasNext()) {
			...
			try {
				...
				maxTime = (int) Math.max(maxTime, addXMLSubTreeForResource(response, rootElement, baseUri,
						apiscolInstanceName, resourceId, editUri));
			} catch (InexistentResourceInDatabaseException e) {
				logger.error(String.format("The resource %s was not found while trying to build xml representation",
						resourceId));
				e.printStackTrace();
			}
	...
	@Override
	public String getResourceStringRepresentation(URI baseUri, String apiscolInstanceName, String resourceId,
			String editUri) throws DBAccessException, InexistentResourceInDatabaseException {
		return formatXML((Document) getResourceRepresentation(baseUri, apiscolInstanceName, resourceId, editUri));
	}
	...
	@Override
	public Document selectResourceFollowingCriterium(URI baseUri, String apiscolInstanceName,
			ISearchEngineResultHandler handler, int start, int rows, String editUri) throws DBAccessException {
		Document response = XMLUtils.createXMLDocument();
		...
		Element spellcheckElement = response.createElement("apiscol:spellcheck");
		...
		while (it.hasNext()) {
			...
			List<String> matchFound = ResourcesKeySyntax.analyseSolrResultId(resultId);
			...
			if (!yetSignaledResource.contains(resourceId))
				try {
					maxTime = Math.max(maxTime, addXMLSubTreeForResource(response, resourcesElement, baseUri,
							apiscolInstanceName, resourceId, editUri));
					addXMLSubTreeForResult(response, hitsElement, resourceId, fileName, score, type, snippets,
							apiscolInstanceName);
		...
		Map<String, List<String>> suggestionsforTerms = handler.getWordSuggestionsByQueryTerms();
		addXMLSubTreeForSpellcheck(response, spellcheckElement, suggestionsforTerms, suggestionsforQuery);
	...
	@Override
	public Document getSuccessfullOptimizationReport(URI baseUri, UriInfo uriInfo) {
		Document report = XMLUtils.createXMLDocument();
		Element rootElement = report.createElement("status");
		Element stateElement = report.createElement("state");
		stateElement.setTextContent("done");
	...
	@Override
	public Document getLinkUpdateProcedureRepresentation(URI baseUri, UriInfo uriInfo) {
		Document report = XMLUtils.createXMLDocument();
		Element rootElement = report.createElement("status");
		Element stateElement = report.createElement("state");
		String state = "";
		...
		Element linkElement = report.createElementNS(UsedNamespaces.ATOM.getUri(), "link");
		linkElement.setAttribute("href", baseUri.toString() + uriInfo.getPath());
		rootElement.appendChild(stateElement);
		rootElement.appendChild(linkElement);
		if (LinkRefreshingHandler.getInstance().getCurrentState() == State.RUNNING) {
			...
			currentUrlElement.setTextContent(LinkRefreshingHandler.getInstance().getCurrentlyParsedUrl());

			rootElement.appendChild(currentUrlElement);
		} else {
			Element terminationElement = report.createElement("termination");
			String termination = "";
			switch (LinkRefreshingHandler.getInstance().getLastProcessTermination()) {
			case SUCCESSFULL:
			...
			errorsElement
					.setTextContent(String.valueOf(LinkRefreshingHandler.getInstance().getLastProcessNumberOfErrors()));
	...
	@Override
	public Document getRecoveryProcedureRepresentation(URI baseUri, UriInfo uriInfo, Integer nbLines) {
		Document report = XMLUtils.createXMLDocument();
		...
		rootElement.appendChild(stateElement);
		rootElement.appendChild(linkElement);
		LinkedList<Pair<String, MessageTypes>> originalMessages = instance.getMessages();
		...
		if (instance.getCurrentState() == ContentRecoveryHandler.State.RECOVERY_RUNNING) {
			Element currentResourceElement = report.createElement("apiscol:current_resource");
			currentResourceElement.setTextContent(instance.getCurrentlyProcessedResource());

			rootElement.appendChild(currentResourceElement);
		} else {
			Element terminationElement = report.createElement("apiscol:termination");
			String termination = "";
			...
			errorsElement.setTextContent(String.valueOf(instance.getLastProcessNumberOfErrors()));

			rootElement.appendChild(terminationElement);
			rootElement.appendChild(errorsElement);
		}
		Element processedElement = report.createElement("apiscol:processed");
		processedElement.setTextContent(String.valueOf(instance.getPercentageOfDocumentProcessed()));
	...
	public Document getSuccessfulGlobalDeletionReport() {
		...
		stateElement.setTextContent("done");
		Element messageElement = report.createElement("message");
		messageElement.setTextContent("All resource have been deleted in content repository.");
		rootElement.appendChild(stateElement);
		rootElement.appendChild(messageElement);
		report.appendChild(rootElement);
		return report;
	...
	@Override
	public Document getThumbListRepresentation(String resourceId, Map<String, Point> thumbsUris, URI baseUri,
		...
		addXMLSubTreeForResource(list, rootElement, baseUri, apiscolInstanceName, resourceId, editUri);
	...
	@Override
	public Document getResourceTechnicalInformations(URI baseUri, String apiscolInstanceName, String resourceId)
			throws ResourceDirectoryNotFoundException, DBAccessException, InexistentResourceInDatabaseException {
		...
		Element technicalLocationElement = infos.createElement("technical-location");
		Element apiscolInstanceElement = infos.createElement("apiscol-instance");
		...
		previewElement.setTextContent(getResourcePreviewUri(baseUri, resourceId));
		rootElement.appendChild(metadataElement);
		...
		sizeElement.setTextContent(getResourceSize(resourceId, resourceDataHandler));
		languageElement.setTextContent(getResourceLanguage(resourceId, resourceDataHandler));
		formatElement.setTextContent(getMimeType(resourceId, resourceDataHandler));
		locationElement.setTextContent(getResourceHTMLUri(baseUri, resourceId));

		technicalLocationElement.setTextContent(getTechnicalLocation(resourceId, resourceDataHandler, baseUri));
	...
	@Override
	public Document getRefreshProcessRepresentation(Integer refreshProcessIdentifier, URI baseUri,
			RefreshProcessRegistry refreshProcessRegistry) {
		Document report = XMLUtils.createXMLDocument();
		Element rootElement = report.createElement("apiscol:status");
		Element stateElement = report.createElement("apiscol:state");
		String resourceId = refreshProcessRegistry.getResourceIdForTransfer(refreshProcessIdentifier);
		States transferState = refreshProcessRegistry.getTransferState(refreshProcessIdentifier);
		stateElement.setTextContent(transferState.toString());
		Element linkElement = report.createElement("link");
		linkElement.setAttribute("href",
				getUrlForRefreshProcess(UriBuilder.fromUri(baseUri), resourceId, refreshProcessIdentifier).toString());
		linkElement.setAttribute("rel", "self");
		linkElement.setAttribute("type", "application/atom+xml");
		Element messageElement = report.createElement("apiscol:message");
		messageElement.setTextContent(refreshProcessRegistry.getMessage(refreshProcessIdentifier));
		rootElement.appendChild(stateElement);
		rootElement.appendChild(linkElement);
		rootElement.appendChild(messageElement);
		Element resourceLinkElement = report.createElement("link");

		resourceLinkElement.setAttribute("href", getResourceHTMLUri(baseUri, resourceId));
