...
import fr.apiscol.database.MongoUtils;
import fr.apiscol.meta.MetadataKeySyntax;
...
public class MongoResourceDataHandler extends AbstractResourcesDataHandler {
	...
	public MongoResourceDataHandler(Map<String, String> dbParams) throws DBAccessException {
		super(dbParams);
	}

	public enum DBKeys {
		id("_id"), relationEntry("relation.resource.identifier.entry"), mainFile("main"), type("type"), metadata(
				"metadata"), url("url"), etag("etag"), deadLink("dead_link"), relation("relation");
	...
	@Override
	protected void dbConnect(Map<String, String> dbParams) throws DBAccessException {
		if (mongo != null) {
			return;
		}
	...
	private void initMetadataCollection() throws DBAccessException {
		metadataCollection = MongoUtils.getCollection(DB_NAME, COLLECTION_NAME, mongo);
	}
	...
	@Override
	public void createMetadataEntry(String metadataId, Document document) throws DBAccessException {
	...
	@Override
	public Node getMetadataHierarchyFromRoot(String rootId, URI baseuri) throws DBAccessException {
		return getMetadataHierarchyFromRoot(rootId, baseuri, 0);
	}

	@SuppressWarnings("unchecked")
	private Node getMetadataHierarchyFromRoot(String rootId, URI baseUri, int depth) throws DBAccessException {
		Node node = new Node();
		node.setMdid(new StringBuilder().append(baseUri.toString()).append('/').append(rootId).toString());
		DBObject rootMetadataObject = getMetadataById(rootId);
		if (rootMetadataObject != null && rootMetadataObject.containsField("relation")) {
			ArrayList<BasicDBObject> relationsObject;
			try {
				relationsObject = (ArrayList<BasicDBObject>) rootMetadataObject.get("relation");
			} catch (ClassCastException e) {
				relationsObject = new ArrayList<BasicDBObject>();
				BasicDBObject relationObject = (BasicDBObject) rootMetadataObject.get("relation");
				relationsObject.add(relationObject);
			}
			if (relationsObject != null)
				for (BasicDBObject relationObject : relationsObject) {
					if (relationObject != null && relationObject.containsField("kind")) {
						DBObject kindObject = (DBObject) relationObject.get("kind");
						if (kindObject.containsField("value")) {
							String value = (String) kindObject.get("value");
							if (StringUtils.equals(value, RelationKinds.CONTIENT.toString())) {
								if (relationObject.containsField("resource")) {
									DBObject resourceObject = (DBObject) relationObject.get("resource");
									if (resourceObject.containsField("identifier")) {
										DBObject identifierObject = (DBObject) resourceObject.get("identifier");
										if (identifierObject.containsField("entry")) {
											String childUri = (String) identifierObject.get("entry");
											String childId = childUri.replaceAll(baseUri.toString(), "").substring(1);
											if (depth < TREE_MAX_DEPTH)
												node.addChild(
														getMetadataHierarchyFromRoot(childId, baseUri, depth + 1));
										}
									}

								}
							}
							if (StringUtils.equals(value, RelationKinds.EST_REQUIS_PAR.toString())) {
								if (relationObject.containsField("resource")) {
									DBObject resourceObject = (DBObject) relationObject.get("resource");
									if (resourceObject.containsField("identifier")) {
										DBObject identifierObject = (DBObject) resourceObject.get("identifier");
										if (identifierObject.containsField("entry")) {
											String nextNodeuri = (String) identifierObject.get("entry");
	...
	@SuppressWarnings("unchecked")
	@Override
	public HashMap<String, String> getMetadataProperties(String metadataId) throws DBAccessException {
		...
		if (metadataObject != null && metadataObject.containsField("general")) {
			...
			if (generalObject != null && generalObject.containsField("description")) {
				DBObject descObject = (DBObject) generalObject.get("description");
				description = getStringInUserLanguage(descObject);
			}
			if (generalObject != null && generalObject.containsField("identifier")) {
				DBObject identifierObject = (DBObject) generalObject.get("identifier");
				...
				if (identifierObject != null && identifierObject.containsField("entry")) {
					String catalog = (String) identifierObject.get("catalog");
					if (catalog != null && catalog.equals("URI")) {
			...
			if (generalObject != null && generalObject.containsField("keyword")) {
				...
				try {
					keywordObjects = (ArrayList<BasicDBObject>) generalObject.get("keyword");
				} catch (ClassCastException e) {
					keywordObjects = new ArrayList<BasicDBObject>();
					BasicDBObject keywordObject = (BasicDBObject) generalObject.get("keyword");
			...
			if (generalObject != null && generalObject.containsField("aggregationLevel")) {
				DBObject aggregationLevelObject = (DBObject) generalObject.get("aggregationLevel");
				if (aggregationLevelObject.containsField("value")) {
					aggregationLevelUri = (String) aggregationLevelObject.get("value");

					aggregationLevelLabel = scolomfrUtils.getSkosApi().getPrefLabelForResource(aggregationLevelUri);
				}
			}
		}
		if (metadataObject != null && metadataObject.containsField("relation")) {
			ArrayList<BasicDBObject> relationsObject;
			try {
				relationsObject = (ArrayList<BasicDBObject>) metadataObject.get("relation");
			} catch (ClassCastException e) {
				relationsObject = new ArrayList<BasicDBObject>();
				BasicDBObject relationObject = (BasicDBObject) metadataObject.get("relation");
				relationsObject.add(relationObject);
			}
			if (relationsObject != null)
				for (BasicDBObject relationObject : relationsObject) {
							...
							if (StringUtils.equals(value, RelationKinds.VIGNETTE.toString())) {
								if (relationObject.containsField("resource")) {
									...
									if (resourceObject.containsField("identifier")) {
										DBObject identifierObject = (DBObject) resourceObject.get("identifier");
										if (identifierObject.containsField("entry")) {
											icon = (String) identifierObject.get("entry");
										}
									}
		...
		if (metadataObject != null && metadataObject.containsField("educational")) {
			ArrayList<BasicDBObject> educationalObjects;
			try {
				educationalObjects = (ArrayList<BasicDBObject>) metadataObject.get("educational");
			} catch (ClassCastException e) {
				educationalObjects = new ArrayList<BasicDBObject>();
				BasicDBObject educationalObject = (BasicDBObject) metadataObject.get("educational");
			...
			if (educationalObjects != null)
				for (BasicDBObject educationalObject : educationalObjects) {
					if (educationalObject != null && educationalObject.containsField("learningResourceType")) {
						...
						try {
							...
							BasicDBObject learningResourceTypeObject = (BasicDBObject) educationalObject
									.get("learningResourceType");
							learningResourceTypeObjects.add(learningResourceTypeObject);
						}
						for (BasicDBObject learningResourceTypeObject : learningResourceTypeObjects) {
							if (learningResourceTypeObject.containsField("value")) {
								educationalResourceTypes.add((String) learningResourceTypeObject.get("value"));
		...
		if (metadataObject != null && metadataObject.containsField("lifeCycle")) {
			DBObject lifeCycleObject = (DBObject) metadataObject.get("lifeCycle");
			if (lifeCycleObject != null && lifeCycleObject.containsField("contribute")) {
				ArrayList<BasicDBObject> contributesObject;
				try {
					contributesObject = (ArrayList<BasicDBObject>) lifeCycleObject.get("contribute");
				} catch (ClassCastException e) {
					contributesObject = new ArrayList<BasicDBObject>();
					BasicDBObject contributeObject = (BasicDBObject) lifeCycleObject.get("contribute");
					contributesObject.add(contributeObject);

				}
				for (BasicDBObject contributeObject : contributesObject) {
					if (contributeObject != null && contributeObject.containsField("role")) {
						DBObject roleObject = (DBObject) contributeObject.get("role");
						...
						if (roleObject.containsField("value")) {
							...
							if (contributeObject.containsField("entity")) {
								try {
									entitiesObjects = (ArrayList<BasicDBObject>) contributeObject.get("entity");
									...
									String entityObject = (String) contributeObject.get("entity");
							...
							for (String entity : entities) {
								...
								if (StringUtils.equals(value, "author") || StringUtils.equals(value, "auteur")
								...
								// duplication
								Pair<String, String> contributor = new Pair<String, String>(value, entity);
		...
		if (metadataObject != null && metadataObject.containsField("technical")) {
			DBObject technicalObject = (DBObject) metadataObject.get("technical");
			if (technicalObject != null && technicalObject.containsField("format")) {
				try {
					contentMime = (String) technicalObject.get("format");
				} catch (ClassCastException e) {
					BasicDBList contentMimes = ((BasicDBList) technicalObject.get("format"));
					contentMime = (String) contentMimes.get(0);
				}
			}
			if (technicalObject != null && technicalObject.containsField("location")) {
				try {
					contentUrl = (String) technicalObject.get("location");
				} catch (ClassCastException e) {
					BasicDBList contentUrls = ((BasicDBList) technicalObject.get("location"));
			...
			if (relationsObject != null)
				for (BasicDBObject relationObject : relationsObject) {
					if (relationObject != null && relationObject.containsField("kind")) {
						...
						if (kindObject.containsField("value")) {
							String value = (String) kindObject.get("value");
							if (StringUtils.equals(value, RelationKinds.FAIT_PARTIE_DE.toString())) {
								if (relationObject.containsField("resource")) {
									DBObject resourceObject = (DBObject) relationObject.get("resource");
									if (resourceObject.containsField("identifier")) {
										DBObject identifierObject = (DBObject) resourceObject.get("identifier");
										if (identifierObject.containsField("entry")) {
											String parentUri = (String) identifierObject.get("entry");
											mdProperties.put(MetadataProperties.parentUri.toString() + parentCount,
													parentUri);
											String parentId = MetadataKeySyntax.extractMetadataIdFromUrl(parentUri);
											if (StringUtils.isNotEmpty(parentId)) {
												DBObject parentObject = getMetadataById(parentId);
												if (parentObject != null && parentObject.containsField("general")) {
													DBObject parentGeneralObject = (DBObject) parentObject
															.get("general");
													if (parentGeneralObject != null
															&& parentGeneralObject.containsField("title")) {
														DBObject parentTitleObject = (DBObject) parentGeneralObject
																.get("title");
														String parentTitle = getStringInUserLanguage(parentTitleObject);
														mdProperties.put(
																MetadataProperties.parentTitle.toString() + parentCount,
																parentTitle);
												...
												logger.error("The URI " + parentUri
		...
		mdProperties.put(MetadataProperties.description.toString(), description);
		for (int i = 0; i < educationalResourceTypes.size(); i++) {
			String educationalresourceType = educationalResourceTypes.get(i);
			mdProperties.put(MetadataProperties.educationalResourceType.toString() + i, educationalresourceType);
			String educationalResourceTypeTiTle = scolomfrUtils.getSkosApi()
					.getPrefLabelForResource(educationalresourceType);
			mdProperties.put(MetadataProperties.educationalResourceTypeTitle.toString() + i,
					educationalResourceTypeTiTle);
		}
		for (int i = 0; i < keywords.size(); i++) {
			mdProperties.put(MetadataProperties.keyword.toString() + i, keywords.get(i));
		}
		mdProperties.put(MetadataProperties.icon.toString(), icon);
		mdProperties.put(MetadataProperties.contentUrl.toString(), contentUrl);
		mdProperties.put(MetadataProperties.contentMime.toString(), contentMime);
		mdProperties.put(MetadataProperties.contentRestUrl.toString(), contentRestUrl);
		mdProperties.put(MetadataProperties.arkIdentifier.toString(), arkIdentifier);
		mdProperties.put(MetadataProperties.aggregationLevel.toString(), aggregationLevelUri);

		mdProperties.put(MetadataProperties.aggregationLevelLabel.toString(), aggregationLevelLabel);
		for (int i = 0; i < authors.size(); i++) {
			mdProperties.put(MetadataProperties.author.toString() + i, authors.get(i));
		}
		for (int i = 0; i < contributors.size(); i++) {
			Pair<String, String> contributor = contributors.get(i);
			String roleUri = contributor.getKey();
			String roleLabel = scolomfrUtils.getSkosApi().getPrefLabelForResource(roleUri);
			if (StringUtils.isEmpty(roleLabel)) {
				roleLabel = roleUri;
			}
			String inlineContributor = new StringBuilder().append(roleUri).append(MetadataProperties.separator)
					.append(roleLabel).append(MetadataProperties.separator).append(contributor.getValue()).toString();
			mdProperties.put(MetadataProperties.contributor.toString() + i, inlineContributor);
		}
		return mdProperties;
	}
	...
	private String getStringInUserLanguage(DBObject langStringObject) {
		...
		if (langStringObject.containsField("string")) {
			...
			try {
				...
				try {
					BasicDBList stringObjects = (BasicDBList) langStringObject.get("string");
	...
	private DBObject getMetadataById(String metadataId) throws DBAccessException {
		...
		try {
			...
			String message = "Error while trying to read in metadata collection " + e.getMessage();
	...
	@Override
	public void loadMetadata(List<String> metadataList) throws DBAccessException {
		BasicDBObject query = new BasicDBObject();
		...
		try {
			...
			if (metadataCursor != null) {
				while (metadataCursor.hasNext()) {
					...
					metadataRepository.put(object.get("_id").toString(), object);
				}
			}
		} catch (MongoException e) {
			String message = "Error while trying to read in metadata collection " + e.getMessage();
	...
	@SuppressWarnings("unchecked")
	@Override
	public HashMap<String, ArrayList<Modification>> getModificationsToApplyToRelatedResources(String url)
			throws DBAccessException {
		BasicDBObject query = new BasicDBObject();
		BasicDBObject fields = new BasicDBObject(DBKeys.id.toString(), true).append(DBKeys.relation.toString(), true);
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (metadataObject != null && metadataObject.containsField(DBKeys.id.toString())) {
					relatedResourceId = (String) metadataObject.get(DBKeys.id.toString());
					...
					if (relationsObject != null)
						for (BasicDBObject relationObject : relationsObject) {
							if (relationObject != null && relationObject.containsField("kind")) {
								DBObject kindObject = (DBObject) relationObject.get("kind");
								if (kindObject.containsField("value")) {
									String relation = (String) kindObject.get("value");
									if (relationObject.containsField("resource")) {
										DBObject resourceObject = (DBObject) relationObject.get("resource");
										if (resourceObject.containsField("identifier")) {
											DBObject identifierObject = (DBObject) resourceObject.get("identifier");
											if (identifierObject.containsField("entry")) {
												String childUri = (String) identifierObject.get("entry");
												if (StringUtils.equals(childUri, url)) {
													if (!modifications.containsKey(relatedResourceId)) {
														modifications.put(relatedResourceId,
																new ArrayList<Modification>());
													}
													RelationKinds relationKind = RelationKinds
															.getFromString(relation.trim());
													if (relationKind != null) {
														Modification modification = new Modification(
																Differencies.removed, relationKind, url);
														if (!modifications.get(relatedResourceId)
																.contains(modification)) {
															modifications.get(relatedResourceId).add(modification);
														}
													}
												}
											}
	...
	@Override
	public void updateMetadataEntry(String metadataId, Document document) throws DBAccessException {
		convertVcardsLines(document);
		String jsonSource = JSonUtils.convertXMLToJson(document);
		DBObject newMetadata = (DBObject) JSON.parse(jsonSource);
		newMetadata.put(DBKeys.id.toString(), metadataId);
		metadataCollection.update(new BasicDBObject().append(DBKeys.id.toString(), metadataId), newMetadata);
		metadataRepository.remove(metadataId);
	}
