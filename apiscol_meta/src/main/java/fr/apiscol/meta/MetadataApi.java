...
import fr.apiscol.meta.dataBaseAccess.DBAccessBuilder.DBTypes;
...
import fr.apiscol.meta.fileSystemAccess.InvalidProvidedMetadataFileException;
import fr.apiscol.meta.fileSystemAccess.MetadataNotFoundException;
import fr.apiscol.meta.fileSystemAccess.ResourceDirectoryInterface;
import fr.apiscol.meta.hierarchy.Deserializer;
import fr.apiscol.meta.hierarchy.HierarchyAnalyser;
import fr.apiscol.meta.hierarchy.Modification;
import fr.apiscol.meta.hierarchy.Node;
import fr.apiscol.meta.representations.EntitiesRepresentationBuilderFactory;
import fr.apiscol.meta.representations.IEntitiesRepresentationBuilder;
import fr.apiscol.meta.representations.XHTMLRepresentationBuilder;
import fr.apiscol.meta.scolomfr.ScolomfrUtilsInitializer;
import fr.apiscol.meta.searchEngine.AbstractSearchEngineFactory;
import fr.apiscol.meta.searchEngine.ISearchEngineFactory;
...
import fr.apiscol.meta.searchEngine.SearchEngineCommunicationException;
import fr.apiscol.meta.searchEngine.SearchEngineErrorException;
import fr.apiscol.transactions.KeyLock;
import fr.apiscol.transactions.KeyLockManager;
import fr.apiscol.utils.TimeUtils;
...
public class MetadataApi extends ApiscolApi {
	...
	public MetadataApi(@Context ServletContext context) throws FileSystemAccessException {
		super(context);
	...
	public static void initializeResourceDirectoryInterface(ServletContext context) throws FileSystemAccessException {

		if (!ResourceDirectoryInterface.isInitialized()) {
			boolean success = ResourceDirectoryInterface.initialize(getProperty(ParametersKeys.fileRepoPath, context),
					getProperty(ParametersKeys.systemDefaultLanguage, context),
					getProperty(ParametersKeys.scolomfrXsdRepoPath, context),
					getProperty(ParametersKeys.temporaryFilesPrefix, context), scolomfrUtils);
			if (!success)
				throw new FileSystemAccessException("File System access initialization failure : check log files.");
		}
	 ...
	 *         graphical representation similar to ApiScol resource portlet.
	 *         </td>
	 *         </tr>
	 *         <tr>
	 *         <td>self</td>
	 ...
	 */
	@POST
	@ApiOperation(value = "Create new metadata entry", notes = "New UUID will be generated for metadata entry", response = Document.class)
	...
	public Response createMetadata(@Context HttpServletRequest request,
			...
			throws FileSystemAccessException, InvalidProvidedMetadataFileException, SearchEngineCommunicationException,
			SearchEngineErrorException, DBAccessException, MetadataNotFoundException {
		...
		try {
			warningMessages = ResourceDirectoryInterface.registerMetadataFile(metadataId, uploadedInputStream, url,
					apiscolInstanceName);
		...
		if (!ResourceDirectoryInterface.commitTemporaryMetadataFile(metadataId)) {
			...
			String error1 = String.format(
					"The file for metadata %s was correctly received and parsed but it was impossible to finalize registration",
					metadataId);
		...
		if (!ResourceDirectoryInterface.commitTemporaryJsonMetadataFile(metadataId)) {
			getLogger().error(String.format(
					"The file for metadata %s was correctly received and parsed but it was impossible to finalize creation of jsonp file",
					metadataId));
		}
		...
		if (aggregationLevel > 0)
			ResourceDirectoryInterface.setAggregationLevel(metadataId, aggregationLevel);
	...
	public Response updateMetadata(@Context HttpServletRequest request,
			@PathParam(value = "mdid") final String metadataId, @FormParam("index") Boolean updateIndex)
			throws FileSystemAccessException, InvalidProvidedMetadataFileException, MetadataNotFoundException,
		...
		try {
			...
			try {
				...
				try {
					...
					response = Response.ok()
							.entity(rb.getMetadataRepresentation(getExternalUri(), apiscolInstanceName, metadataId,
									true, false, true, -1, Collections.<String, String>emptyMap(), resourceDataHandler,
									editUri))
			...
			getLogger()
					.info(String.format("Leaving critical section with mutual exclusion for metadata %s", metadataId));
		}
	...
	public Response changeMetadata(@Context HttpServletRequest request,
			@PathParam(value = "mdid") final String metadataId, @FormDataParam("file") InputStream uploadedInputStream,
			@FormDataParam("file") FormDataContentDisposition fileDetail,
		...
		try {
			...
			try {
				...
				try {
					warningMessages = ResourceDirectoryInterface.registerMetadataFile(metadataId, uploadedInputStream,
							url, apiscolInstanceName);
				} catch (FileSystemAccessException e) {
					getLogger().error(String.format(
				...
				if (!successFullFileDeletion) {
					...
					// exist
					String errorReport = String.format("Failed to delete file for metadata %s", metadataId);
				...
				if (continueProcessing) {
					fileCommitSuccessfull = ResourceDirectoryInterface.commitTemporaryMetadataFile(metadataId)
							&& ResourceDirectoryInterface.commitTemporaryJsonMetadataFile(metadataId);
				...
				if (!fileCommitSuccessfull) {
					...
					if (successFullFileDeletion)
						errorReport = String.format(
								"The file has been deleted but we failed to  replace it with the new one. Metadata  %s are damaged please send the file again.",
								metadataId);
					else
						errorReport = String.format(
								"The file has not been deleted and we failed to  replace it with the new one. Metadata  %s have not changed.",
								metadataId);
					warnings.append(errorReport);
					getLogger().error(errorReport);
					// delete the temporary file
	...
	private void updateMetadataEntryInDataBase(String metadataId) throws DBAccessException, MetadataNotFoundException {
		IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
				.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
		Document metadata = ResourceDirectoryInterface.getMetadataAsDocument(metadataId);
		resourceDataHandler.updateMetadataEntry(metadataId, metadata);

	}

	private Node getMetadataHierarchyFromRoot(String metadataId) throws DBAccessException, MetadataNotFoundException {
		IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
				.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
		return resourceDataHandler.getMetadataHierarchyFromRoot(metadataId, getExternalUri());

	}

	private void createMetadataEntryInDatabase(String metadataId) throws DBAccessException, MetadataNotFoundException {
		IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
				.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
		Document metadata = ResourceDirectoryInterface.getMetadataAsDocument(metadataId);
	...
	private void deleteMetadataEntryInDatabase(String metadataId) throws DBAccessException, MetadataNotFoundException {
		IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
				.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
	...
	private HashMap<String, ArrayList<Modification>> getModificationsToApplyToRelatedResources(String url)
			throws DBAccessException, MetadataNotFoundException {
		IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
				.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
		return resourceDataHandler.getModificationsToApplyToRelatedResources(url);
	...
	@Produces({ MediaType.APPLICATION_ATOM_XML, MediaType.APPLICATION_XML })
	public Response updateTechnicalInformations(@Context HttpServletRequest request,
			...
			@DefaultValue("") @FormParam("format") String format, @DefaultValue("") @FormParam("thumb") String thumb,
			@DefaultValue("") @FormParam("preview") String preview, @DefaultValue("") @FormParam("size") String size,
			@DefaultValue("") @FormParam("language") String language,
			@DefaultValue("") @FormParam("edit_uri") String editUri) throws FileSystemAccessException,
			InvalidProvidedMetadataFileException, MetadataNotFoundException, InvalidEtagException, DBAccessException {
		takeAndReleaseGlobalLock();
		KeyLock keyLock = null;
		try {
			keyLock = keyLockManager.getLock(metadataId);
			...
			try {
				...
				if (!StringUtils.isEmpty(editUri))
					MetadataApi.editUri = editUri;
				checkFreshness(request.getHeader(HttpHeaders.IF_MATCH), metadataId);
				ResourceDirectoryInterface.updateTechnicalInformation(metadataId, size, language, technicalLocation,
	 ...
	 *            Each one is structured following the pattern : element::value
	 *            <br/>
	...
	public Response getMetadataList(@Context HttpServletRequest request,
			@QueryParam(value = "format") final String format, @QueryParam(value = "query") String query,
			@QueryParam(value = "mdid") String forcedMetadataId, @QueryParam(value = "mdids") String forcedMetadataIds,
		...
		if (StringUtils.isNotEmpty(forcedMetadataId)) {
			return forcedMetadataSearch(rb, request, forcedMetadataId, includeDescription, includeTimestamp);
		}
		if (StringUtils.isNotEmpty(forcedMetadataIds)) {
			...
			return forcedMetadataListSearch(rb, request, forcedMetadataIdList, includeDescription, includeTimestamp);
		}
		for (int i = 0; i < supplementsIds.length; i++) {
			String metadataId = MetadataKeySyntax.extractMetadataIdFromUrl(supplementsIds[i]);
		...
		if (StringUtils.isNotEmpty(additiveStaticFilters))
			try {
				additiveStaticFiltersList = new Gson().fromJson(additiveStaticFilters, collectionType);
			} catch (Exception e) {
				String message = String.format("The list of additive static filters %s is impossible to parse as JSON",
		...
		if (StringUtils.isNotEmpty(dynamicFilters))
			try {
				dynamicFiltersList = new Gson().fromJson(dynamicFilters, collectionType);
			} catch (Exception e) {
				String message = String.format("The list of dynamic filters %s is impossible to parse as JSON",
						dynamicFilters);
				getLogger().warn(message);
				throw new InvalidFilterListException(message);
			}
		...
		if (StringUtils.isNotEmpty(additiveDynamicFilters))
			try {
				additiveDynamicFiltersList = new Gson().fromJson(additiveDynamicFilters, collectionType);
			} catch (Exception e) {
				String message = String.format("The list of additive dynamic filters %s is impossible to parse as JSON",
		...
		Object result = searchEngineQueryHandler.processSearchQuery(query.trim(), supplementsIds, fuzzy,
				staticFiltersList, additiveStaticFiltersList, dynamicFiltersList, additiveDynamicFiltersList,
				disableHighlighting, start, rows, sort);
		...
		if (includeDescription) {
			try {
				resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils).setDbType(DBTypes.mongoDB)
		...
		Response response = Response
				.ok(rb.selectMetadataFollowingCriterium(getExternalUri(), uriInfo.getPath(), apiscolInstanceName,
						apiscolInstanceLabel, handler, start, rows, includeDescription, includeTimestamp,
						resourceDataHandler, editUri, version), rb.getMediaType())
				.header("Access-Control-Allow-Origin", "*").build();
		return response;

	}

	private Response forcedMetadataSearch(IEntitiesRepresentationBuilder<?> rb, HttpServletRequest request,
			String forcedMetadataId, boolean includeDescription, boolean includeTimestamp)
			throws NumberFormatException, DBAccessException, SearchEngineErrorException {
		Object result = searchEngineQueryHandler.processSearchQuery(forcedMetadataId);
		ISearchEngineResultHandler handler = searchEngineFactory.getResultHandler();
		handler.parse(result);
		IResourceDataHandler resourceDataHandler = null;
		if (includeDescription) {
		...
		return Response.ok(rb.selectMetadataFollowingCriterium(getExternalUri(), uriInfo.getPath(), apiscolInstanceName,
				apiscolInstanceLabel, handler, 0, 1, includeDescription, includeTimestamp, resourceDataHandler, editUri,
				version), rb.getMediaType()).build();
	}

	private Response forcedMetadataListSearch(IEntitiesRepresentationBuilder<?> rb, HttpServletRequest request,
			List<String> forcedMetadataIdList, boolean includeDescription, boolean includeTimestamp)
			throws SearchEngineErrorException, NumberFormatException, DBAccessException {
		Object result = searchEngineQueryHandler.processSearchQuery(forcedMetadataIdList);
		ISearchEngineResultHandler handler = searchEngineFactory.getResultHandler();
		handler.parse(result);
		IResourceDataHandler resourceDataHandler = null;
		if (includeDescription) {
		...
		return Response.ok(rb.selectMetadataFollowingCriterium(getExternalUri(), uriInfo.getPath(), apiscolInstanceName,
				apiscolInstanceLabel, handler, 0, forcedMetadataIdList.size(), includeDescription, includeTimestamp,
				resourceDataHandler, editUri, version), rb.getMediaType()).build();
	}
	...
	private Response getMetadataRepresentation(HttpServletRequest request, HttpServletResponse httpServletResponse,
		...
		IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
				.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
		Object response = rb.getMetadataRepresentation(getExternalUri(), apiscolInstanceName, metadataId,
				includeDescription, includeHierarchy, includeTimestamp, maxDepth, params, resourceDataHandler, editUri);

		String mediaType = rb.getMediaType().toString();
		return Response.ok(response, mediaType)
				.header(HttpHeaders.ETAG,
						TimeUtils.toRFC3339(Long.parseLong(ResourceDirectoryInterface.getTimeStamp(metadataId))))
				.build();
	}

	@GET
	@Path("/{mdid}")
	@Produces({ MediaType.TEXT_HTML, MediaType.APPLICATION_XHTML_XML, MediaType.APPLICATION_ATOM_XML,
			MediaType.APPLICATION_XML, "application/javascript" })
	public Response getMetadata(@Context HttpServletRequest request, @Context HttpServletResponse httpServletResponse,
			@PathParam(value = "mdid") final String metadataId,
	...
	@GET
	@Path("/{mdid}/snippet")
	@Produces({ MediaType.APPLICATION_XML, MediaType.TEXT_HTML, MediaType.APPLICATION_XHTML_XML,
	...
	public Response getMetadataSnippet(@Context HttpServletRequest request,
		...
		IEntitiesRepresentationBuilder<?> rb = EntitiesRepresentationBuilderFactory
				.getRepresentationBuilder(requestedFormat, context);
		rb.setScolomfrUtils(scolomfrUtils);
		Object response = rb.getMetadataSnippetRepresentation(getExternalUri(), apiscolInstanceName, metadataId,
				version);

		String mediaType = rb.getMediaType().toString();
		return Response.ok(response, mediaType)
				.header(HttpHeaders.ETAG, ResourceDirectoryInterface.getTimeStamp(metadataId)).build();
	}
	...
	public Response getQuerySuggestions(@Context HttpServletRequest request,
		...
		if (StringUtils.isBlank(query))
			...
			handler.parse(result);
			return Response.ok(
					rb.selectMetadataFollowingCriterium(getExternalUri(), uriInfo.getPath(), apiscolInstanceName,
							apiscolInstanceLabel, handler, 0, 10, false, false, null, editUri, version),
		...
		try {
			...
			try {
				...
				if (response == null) {
					...
					response = Response.ok(rb.getMetadataSuccessfulDestructionReport(getExternalUri(),
							apiscolInstanceName, metadataId, warnings.toString()), rb.getMediaType());
	...
	private void checkFreshness(String providedEtag, String metadataId)
		...
		String storedEtag = TimeUtils.toRFC3339(Long.parseLong(ResourceDirectoryInterface.getTimeStamp(metadataId)));
		if (!StringUtils.equals(providedEtag, storedEtag))
			throw new InvalidEtagException(metadataId, providedEtag);

	}

	private void checkMdidSyntax(String metadataId) throws IncorrectMetadataKeySyntaxException {
	...
	private void initializeStaticParameters() {
		apiscolInstanceName = getProperty(ParametersKeys.apiscolInstanceName, context);
		apiscolInstanceLabel = getProperty(ParametersKeys.apiscolInstanceLabel, context);
	}
	...
	private void createSearchEngineQueryHandler(ServletContext context) {
		...
		String solrSearchPath = getProperty(ParametersKeys.solrSearchPath, context);
		String solrUpdatePath = getProperty(ParametersKeys.solrUpdatePath, context);
		String solrExtractPath = getProperty(ParametersKeys.solrExtractPath, context);
		String solrSuggestPath = getProperty(ParametersKeys.solrSuggestPath, context);
		try {
	...
	public Response defineRelations(@Context HttpServletRequest request,
			@PathParam(value = "mdid") final String metadataId, @FormParam(value = "hierarchy") final String hierarchy)
			throws FileSystemAccessException, InvalidProvidedMetadataFileException, MetadataNotFoundException,
			InvalidEtagException, DBAccessException, JDOMException, IOException, URISyntaxException {
		String requestedFormat = request.getHeader(HttpHeaders.ACCEPT);
		IEntitiesRepresentationBuilder<?> rb = EntitiesRepresentationBuilderFactory
				.getRepresentationBuilder(requestedFormat, context);
		rb.setScolomfrUtils(scolomfrUtils);
		...
		try {
			...
			try {
				...
				HashMap<String, ArrayList<Modification>> modifications = HierarchyAnalyser.getModifications();
				ResourceDirectoryInterface.applyChanges(modifications, getExternalUri());
				refresModifiedMetadataInDatabase(modifications);

				IResourceDataHandler resourceDataHandler = new DBAccessBuilder().setScolomfrUtils(scolomfrUtils)
						.setDbType(DBTypes.mongoDB).setParameters(getDbConnexionParameters()).build();
				Object representation = rb.getMetadataRepresentation(getExternalUri(), apiscolInstanceName, metadataId,
						false, true, false, -1, Collections.<String, String>emptyMap(), resourceDataHandler,
						requestedFormat);
			...
			getLogger().info(
					String.format("Leaving critical section with mutual exclusion for whole repository", metadataId));
		}
		return response.build();
	}

	private void refresModifiedMetadataInDatabase(HashMap<String, ArrayList<Modification>> modifications)
			throws DBAccessException, MetadataNotFoundException {
		...
		while (metadataIterator.hasNext()) {
			...
			if (metadataUri.contains(getExternalUri().toString())) {
				metadataUri = metadataUri.replaceAll(getExternalUri().toString(), "").substring(1);
			}
	...
	private void refreshMetadata(String metadataId, boolean takeCareOfDatabase, boolean takecareOfSearchEngine,
			boolean takeCareOfJsonpFile) throws DBAccessException, MetadataNotFoundException {
	...
	protected void fetchScolomfrUtils(ServletContext context) {
		scolomfrUtils = (Scolomfr3Utils) context.getAttribute(ScolomfrUtilsInitializer.ENVIRONMENT_PARAMETER_KEY);
		if (!(scolomfrUtils instanceof Scolomfr3Utils)) {
			getLogger().error("Impossible to fetch instance of SkosVocabulary from servlet context");
		}
		getLogger().info("Successfully fetched instance of SkosVocabulary from servlet context");
	}
